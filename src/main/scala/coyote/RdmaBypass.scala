...
* Bypass
* */
case class RdmaBypass() extends Component with SetDefaultIO with RenameIO {
  ...
  val fsm = new StateMachine {
    ...
    IDLE.whenIsActive{
      when(mode(0)){
        ...
        for (e <- List(cntSent, cntRec, cntSqSent, cntAxiToSend, cntWord, clk))
          e.clearAll()
      }
    }
    ...
    WORK_MSTR.whenIsActive{
      ...
      when(cntAxiToSend>0){
        ...
        when(cntWord === ((len>>6)-1)){io.axis_rdma_1_src.tlast.set()}
        when(io.axis_rdma_1_src.fire){
          when(cntWord === ((len>>6)-1)){cntWord.clearAll()} otherwise {cntWord := cntWord + 1}
        }
      }

      when(cntSent===cnt || clk===clkTimeOut){
        mode := 0
        goto(IDLE)
      }
    }
    ...
    WORK_SLVE.whenIsActive{
      ...
      // time out
      when(cntRec===cnt || clk===clkTimeOut){
        mode := 0
        goto(IDLE)
      }
    }
