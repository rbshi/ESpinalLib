...
//  val ap_ready = ctlReg.createReadOnly(Bool(), 0, 3).init(False)
  ...
  val reduced_opdone = opGrp.map(_.io.done).foldLeft(False)(_ && _)

  // ap_done
  when(~ap_done){ap_done := reduced_opdone}
  when(ap_start){ap_idle := False}
  when(~ap_done && reduced_opdone){ap_idle := True}
